...
<div class="main-panel">
    <div class="content-wrapper" style="background-color: #D1D1D1;">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h3 class="card-title text-black">Buat Anggaran Rumah Tangga Baru</h3>
            <div class="mb-3">
                <button onclick="window.location.href='{{ route('anggaran-rumah-tangga') }}'" class="btn btn-primary">
                    ← Kembali ke Anggaran Rumah Tangga
                </button>
            </div>
        </div>
        @if ($errors->any())
        ...
        <div class="col grid-margin stretch-card">
            <div class="card" style="background-color: #2A2A2A;">
                <div class="card-body">
                    <form action="{{ route('save-anggaran-rumah-tangga') }}" method="POST">
                        @csrf

                        <textarea id="myeditorinstance" name="anggaran_rumah_tangga">{{{ $anggaranRumahTangga }}}</textarea>

                        <div class="m-2 float-right">
                            <button type="submit" class="btn btn-primary">Simpan Anggaran Rumah Tangga</button>
                        </div>
                    </form>
                </div>
            </div>
