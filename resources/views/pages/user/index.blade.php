                            ...
                            <table class="table table-hover text-white">
                                <thead>
                                    <tr style="background-color: #D1D1D1;">
                                        ...
                                        <th style="color: black;">Is Membership</th>
                                        <th style="color: black;">Membership Start</th>
                                        <th style="color: black;">Membership End</th>
                                        <th style="color: black;">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($user as $item)
                                        <tr onclick="location.href='{{ route('anggota.show', $item->id) }}'"
                                            ...
                                            <td class="text-center">{{ $item->membership_start ?? '-' }}</td>
                                            <td class="text-center">{{ $item->membership_end ?? '-' }}</td>
                                            <td>
                                                <a href="{{ route('anggota.edit', $item->id) }}"
                                                    class="btn btn-sm btn-warning">Edit</a>

                                                <form action="{{ route('anggota.toggleSuspend', $item->id) }}"
                                                    method="POST" style="display: inline-block;">
                                                    @csrf
                                                    <button type="submit"
                                                        class="btn btn-sm {{ $item->status == 1 ? 'btn-secondary' : 'btn-success' }}"
